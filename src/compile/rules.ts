 ...
 */
export function deriveRules(scan: ScanResult): Rule[] {
  const rules: Rule[] = [];
  ...
  if (scan.components.length > 0) {
    const withVariants = scan.components.filter(
    ...
    rules.push({
      ...
      severity: "error",
      statement:
        "Use the project's existing components instead of writing raw HTML elements for anything the system already covers. " +
    ...
    if (withVariants.length > 0) {
      rules.push({
        ...
        fix: withVariants
          ...
          .map(
            (c) =>
              `${c.name}: ${Object.entries(c.variants)
  ...
  rules.push({
    ...
    pattern:
      "\\b(seamless(?:ly)?|effortless(?:ly)?|unlock|revolutioniz\\w*|supercharge\\w*|game[- ]changing|delightful)\\b",
    patternFlags: "gi",
