 ...
 */
function extractBlock(source: string, openBraceIndex: number): string | null {
  let depth = 0;
    ...
    for (let i = 0; i < block.length; i++) {
      ...
      if (quote) {
        ...
        if (ch === quote) {
          ...
          if (depth === 0) keyBuffer = stringBuffer;
          stringBuffer = "";
        } else {
          stringBuffer += ch;
        }
      ...
      if (depth === 0) keyBuffer += ch;
    }

    if (values.length) result[propName] = values;
...
function parseDefaults(cvaBody: string): Record<string, string> {
  ...
  let m: RegExpExecArray | null;
  while ((m = pairRe.exec(block)) !== null) out[m[1]] = m[2];
  return out;
}
...
export async function scanComponents(
  repoRoot: string,
  alias: string | null
): Promise<{ components: ComponentContract[]; hasShadcn: boolean }> {
  ...
  for (const file of files) {
    ...
    if (/\/components\/ui\//.test(normalized)) hasShadcn = true;
    ...
    while ((m = NAMED_EXPORT.exec(content)) !== null) {
      ...
      if (/Variants$/.test(name) || seen.has(name)) continue;
      seen.add(name);
