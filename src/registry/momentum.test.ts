...
const rel = {
  ...
  signals: [],
  primaryDomain: "visual" as const,
  domains: [],
};

function entry(id: string, installs: number, state = "steady"): SnapshotEntry {
  return {
    id,
    slug: id.split("/").at(-1)!,
    name: id.split("/").at(-1)!,
...
test("merging views never overwrites cumulative installs with a windowed rate", () => {
  ...
  const allTime = [
    { id: "a/b/x", slug: "x", name: "x", source: "a/b", installs: 770700, sourceType: "github" as const, installUrl: null, url: "u" },
  ];
  const trending = [
    { id: "a/b/x", slug: "x", name: "x", source: "a/b", installs: 4100, sourceType: "github" as const, installUrl: null, url: "u" },
  ];
...
test("skills only in trending get null installs, not a fake count", () => {
  const trending = [
    { id: "new/repo/y", slug: "y", name: "y", source: "new/repo", installs: 3000, sourceType: "github" as const, installUrl: null, url: "u" },
  ];
...
test("content hash ignores rank churn", () => {
  ...
  assert.equal(
    hashEntries([a]),
    hashEntries([b]),
    "rank movement alone must not look like a content change"
  );
});
