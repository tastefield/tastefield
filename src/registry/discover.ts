...
import { scoreDomains, type Domain, type DomainMatch } from "./domains.js";
 ...
 */
export const DESIGN_OWNERS: Array<{ owner: string; why: string }> = [
 ...
 */
export const QUERY_BATTERY: string[] = [
  // visual
  "design system",
  "design tokens",
 ...
 * excluded pile.
 */
export function scoreRelevance(input: RelevanceInput): RelevanceScore {
...
export async function harvest(
  ...
  options: { queries?: string[]; owners?: string[]; perQuery?: number } = {}
  ...
  const record = (skill: V1Skill, via: string, isOwner: boolean): void => {
  ...
  for (const query of queries) {
    try {
      ...
      for (const skill of res.data) record(skill, `search:${query}`, false);
    } catch (err) {
  ...
  for (const owner of owners) {
    try {
      ...
      for (const skill of res.data) record(skill, `owner:${owner}`, true);
    } catch (err) {
  ...
  const candidates = [...found.values()].sort(
    (a, b) =>
      b.relevance.score - a.relevance.score || b.installs - a.installs
  );
