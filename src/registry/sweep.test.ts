...
import { sweepAll, enrichCandidates, shardByTier, projectCost, type TieredSkill } from "./sweep.js";

function skill(id: string, name = id.split("/").at(-1)!, installs = 100) {
  return {
    id,
    slug: name,
    name,
...
function pagedClient(pages: Array<ReturnType<typeof skill>[]>, total: number) {
  let call = 0;
...
test("cost projection matches the real registry size", () => {
  ...
  assert.ok(
    cost.enrichMinutes > cost.sweepMinutes,
    "enrichment, not sweeping, is the expensive stage"
  );
...
test("sweep paginates to completion and keeps only design candidates", async () => {
  ...
  assert.deepEqual(
    result.candidates.map((c) => c.slug).sort(),
...
test("sweep checkpoints and resumes without losing work", async () => {
  ...
  assert.deepEqual(
    second.candidates.map((c) => c.slug).sort(),
    ["design-tokens", "ui-kit"],
    "resumed run retains earlier candidates"
  );
});
...
test("enrichment budget degrades to known instead of dropping skills", async () => {
          ...
          files: [
            {
              path: "SKILL.md",
              contents:
                "Extract color, typography, spacing and component tokens into a design system with a consistent visual style.",
            },
          ],
        }),
        { status: 200, headers: { "content-type": "application/json" } }
  ...
  assert.equal(result.stats.requested, 1);
  assert.equal(
    result.enriched.filter((e) => e.tier === "known").length,
    2,
    "over-budget candidates fall back to known"
  );
});
