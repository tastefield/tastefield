...
function matchTerms(raw: string, terms: string[]): string[] {
...
export function scoreDomains(input: DomainScoreInput): DomainScore {
  ...
  for (const def of DOMAINS) {
    ...
    if (weakAll.length) score += Math.min(weakAll.length * 3, 12);
  ...
  if (ownerBoost) signals.push(`owner:+${ownerBoost}`);
...
export function domainRationale(domain: Domain): string {
