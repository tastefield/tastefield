...
import { fileHash, loadComposition, loadSkillFrontmatter, writeYaml, } from "../utils/fs.js";
export function resolveCompositionPath(fieldRoot, nameOrPath) {
...
export function composeComposition(fieldRoot, nameOrPath) {
    ...
    for (const skill of composition.skills) {
        ...
        if (frontmatter.name !== skill.name) {
            throw new Error(`Skill name mismatch: composition declares "${skill.name}" but SKILL.md has "${frontmatter.name}"`);
        }
    ...
    const lockPath = join(fieldRoot, "compositions", `${composition.id}.lock.json`);
