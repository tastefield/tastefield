...
export function initField(opts) {
    const name = opts.name ?? "my-field";
    const id = opts.id ?? name;
    const root = opts.dir;
    if (existsSync(join(root, "field.yaml")) && !opts.force) {
        throw new Error(`Field already exists at ${root}. Use --force to overwrite templates.`);
    }
    const dirs = [
        "sources",
        "skills",
    ...
    const methodTpl = readFileSync(join(tpl, "method.yaml"), "utf8")
        ...
        .replace("Outcome this Method produces.", "Starter Method — replace with your outcome.");
    writeFileSync(join(root, "methods", "starter.yaml"), methodTpl, "utf8");
    const evalsTpl = readFileSync(join(tpl, "evals.json"), "utf8").replaceAll("example-method", "starter");
    ...
    writeFileSync(join(root, "sources", "README.md"), "# Sources\n\nAttach markdown, research, and creative reference material here.\n", "utf8");
    return field;
}
export function readField(fieldRoot) {
    return loadField(fieldRoot);
}
export function addSkillRef(fieldRoot, ref) {
...
export function addSource(fieldRoot, sourcePath) {
    const field = loadField(fieldRoot);
