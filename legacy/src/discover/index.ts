...
function uniquePaths(specs: RootSpec[]): RootSpec[] {
...
export function defaultSkillRoots(opts: DiscoverOptions = {}): RootSpec[] {
...
function walkAgentSkillRoots(cwd: string): RootSpec[] {
...
export function discoverSkills(opts: DiscoverOptions = {}): DiscoveredSkill[] {
  const cwd = opts.cwd ?? process.cwd();
  ...
  const byNameAndPath = new Map<string, DiscoveredSkill>();

  for (const root of roots) {
    if (!existsSync(root.path) || !statSync(root.path).isDirectory()) continue;
    ...
    for (const skillMd of skillMdFiles) {
      ...
      try {
        ...
        if (byNameAndPath.has(key)) continue;
        ...
        byNameAndPath.set(key, {
          ...
          version:
            typeof versionMeta === "string" ? versionMeta : undefined,
        });
      } catch {
  ...
  return [...byNameAndPath.values()].sort((a, b) =>
    a.name.localeCompare(b.name),
  );
}

export function formatSkillTable(skills: DiscoveredSkill[]): string {
  if (skills.length === 0) {
