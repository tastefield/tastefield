...
function orderedSkills(method) {
    if (method.precedence.length === 0) {
        const rank = { override: 0, primary: 1, supporting: 2 };
        return [...method.skills].sort((a, b) => rank[a.role] - rank[b.role] || a.name.localeCompare(b.name));
    }
    ...
    for (const skill of method.skills) {
        if (!method.precedence.includes(skill.name))
            ordered.push(skill);
    }
    return ordered;
}
function buildOrchestratorMarkdown(method, relativeRules, skillEntries, target) {
    const skillList = skillEntries
        .map((s) => `   - **${s.name}** (${s.role}, v${s.version}) — load \`${s.relativeSkillMd}\``)
        .join("\n");
    const targetNote = target === "generic"
...
| --- | --- | --- | --- |
${skillEntries
        .map((s) => `| ${s.name} | ${s.role} | ${s.version} | \`${s.relativeDir}\` |`)
        .join("\n") || "| _(none)_ | | | |"}
...
export function exportMethod(opts) {
    ...
    writeFileSync(join(outDir, "references", "field-map.md"), buildFieldMap(method), "utf8");
    const evalsSrc = join(opts.fieldRoot, method.evals);
    if (existsSync(evalsSrc)) {
        mkdirSync(join(outDir, "evals"), { recursive: true });
        writeFileSync(join(outDir, "evals", "evals.json"), readText(evalsSrc), "utf8");
    }
    const skillMdBody = buildOrchestratorMarkdown(method, "references/rules.md", skillEntries, target);
    const skillMdPath = join(outDir, "SKILL.md");
    writeFileSync(skillMdPath, skillMdBody, "utf8");
    writeFileSync(join(outDir, "tastefield.export.json"), JSON.stringify({
        method_id: method.id,
        method_version: method.version,
        target,
...
export function defaultExportDir(fieldRoot, methodId, target) {
    return join(fieldRoot, "exports", `${methodId}-${target}`);
}
export function suggestInstallPath(methodId, target) {
    switch (target) {
        case "cursor":
...
export function exportLabel(outDir) {
