 ...
 */
async function readAlias(repoRoot: string): Promise<string | null> {
...
async function detectFramework(
  repoRoot: string
): Promise<ScanResult["stack"]["framework"]> {
 ...
 */
async function detectTailwindV3Config(repoRoot: string): Promise<boolean> {
  for (const name of [
    "tailwind.config.js",
    "tailwind.config.ts",
    "tailwind.config.mjs",
    ...
    if (await exists(path.join(repoRoot, name))) return true;
  }
  return false;
}
...
export async function scan(repoRoot: string): Promise<ScanResult> {
  ...
  if (!tailwind && hasV3Config) tailwind = "v3";

  if (css.tokens.length === 0) {
    warnings.push(
      "No CSS custom properties found. Tastefield reads design tokens from `--token: value` declarations; " +
        "if your tokens live only in a tailwind.config.js theme object, they won't be picked up in this version."
    );
  }
  if (components.length === 0) {
    warnings.push(
      "No components found. Looked for .tsx/.jsx files under a components/ directory or containing cva() variant definitions."
    );
  }
  if (tailwind === "v3" && css.tokens.length === 0) {
    warnings.push(
      "Detected a Tailwind v3 config. Theme values declared in JS config are not read (the config is never executed, by design)."
    );
  }
