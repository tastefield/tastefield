...
async function cmdInit(repoRoot: string, register: boolean): Promise<number> {
  ...
  const contractCount = ctx.components.filter(
    (comp) => Object.keys(comp.variants).length > 0
  ).length;
  ...
  process.stdout.write(
    `${c.green("✔")} ${tokenCount} design tokens\n` +
  ...
  if (register) {
    ...
    for (const r of registrations) {
      const verb =
        r.action === "already-present" ? "already registered in" : `${r.action} `;
      process.stdout.write(
        `${c.green("✔")} ${r.agent} — ${verb}${c.dim(r.file)}\n`
      );
    }
  }
...
async function cmdCheck(
  ...
  if (!rules.length) {
    process.stderr.write(
      `${c.red("✖")} No compiled standards found. Run ${c.bold("tastefield init")} first.\n`
    );
    return 1;
  }
  ...
  if (!files.length) {
    process.stderr.write(
      `${c.red("✖")} No files given. Usage: tastefield check src/**/*.tsx\n`
    );
    return 1;
  }
  ...
  for (const file of files) {
    ...
    for (const v of violations) {
      ...
      process.stdout.write(
        `  ${c.dim(String(v.line).padStart(4))}  ${tag}  ${v.ruleId}  ${c.dim(v.excerpt)}\n`
      );
      if (v.fix) process.stdout.write(`        ${c.dim(v.fix)}\n`);
...
async function cmdSkills(
  ...
  switch (sub) {
    case "list": {
      ...
      process.stdout.write(
        c.dim(
          `${"SKILL".padEnd(30)}${"SOURCE".padEnd(38)}${"CATEGORY".padEnd(11)}${"INSTALLS".padStart(9)}\n`
        )
      );

      for (const s of skills) {
        process.stdout.write(
          `${s.slug.padEnd(30)}${c.dim(s.source.padEnd(38))}${s.category.padEnd(11)}${formatInstalls(s.installs).padStart(9)}\n`
        );
      }

      process.stdout.write(`\n${note}\n`);
      process.stdout.write(
        c.dim(`Install counts shown as "—" are unknown, not zero.\n`)
      );
      return 0;
    }
    ...
    case "recipes": {
      ...
      for (const { name, skills } of recipes()) {
        ...
        for (const s of skills) {
          process.stdout.write(
            `  ${s.slug.padEnd(30)}${c.dim(s.officialSummary ?? s.rationale)}\n`
          );
        }
        process.stdout.write("\n");
      }
    ...
    case "import": {
      ...
      if (!target) {
        process.stderr.write(
          `${c.red("✖")} Usage: tastefield skills import <id|slug>\n`
        );
        return 1;
      }
      ...
      if (!curated) {
        process.stderr.write(
          `${c.red("✖")} "${target}" is not in the curated set.\n` +
      ...
      process.stdout.write(
        `${c.green("✔")} ${record.files.length} file(s) → ${c.cyan(record.dir)}\n` +
    ...
    case "imported": {
      ...
      if (!imported.length) {
        process.stdout.write(
          `${c.dim("No skills imported yet. Try `tastefield skills import polish`.")}\n`
        );
        return 0;
      }
      for (const s of imported) {
        process.stdout.write(
          `${c.green("✔")} ${s.slug.padEnd(30)}${c.dim(`${s.files.length} file(s) · via ${s.via} · ${s.importedAt.slice(0, 10)}`)}\n`
        );
      }
      return 0;
    }
    ...
    case "discover": {
      process.stdout.write(
        `${c.dim("Harvesting candidates from search battery + owner allowlist…")}\n`
      );
      ...
      if (result.errors.length && result.candidates.length === 0) {
        process.stderr.write(
          `${c.red("✖")} Discovery needs a Vercel OIDC token.\n` +
      ...
      const shortlist = result.candidates.filter(
        (x) => x.relevance.verdict !== "exclude"
      );

      process.stdout.write(
        c.dim(`${"SCORE".padStart(5)}  ${"VERDICT".padEnd(8)}${"SKILL".padEnd(32)}${"SOURCE".padEnd(34)}INSTALLS\n`)
      );
      for (const x of shortlist.slice(0, 60)) {
        const tag =
          x.relevance.verdict === "include" ? c.green("include ") : c.yellow("review  ");
        process.stdout.write(
          `${String(x.relevance.score).padStart(5)}  ${tag}${x.slug.slice(0, 31).padEnd(32)}${c.dim(x.source.slice(0, 33).padEnd(34))}${formatInstalls(x.installs).padStart(8)}\n`
        );
      }
    ...
    case "export": {
      ...
      const payload = {
        ...
        installsNote:
          "null means the count was not verified. Render as an em dash, never as 0.",
      ...
      if (out) {
        ...
        if (error) process.stdout.write(`${c.yellow("!")} ${error}\n`);
      } else {
