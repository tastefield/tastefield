...
test("every curated design skill scores as include or review", () => {
  for (const s of CURATED) {
    ...
    assert.notEqual(
      result.verdict,
      "exclude",
      `${s.slug} scored ${result.score} (${result.signals.join(", ")}) — a known design skill must not be excluded`
    );
  }
});
...
test("known design skills outscore known infrastructure skills", () => {
  ...
  for (const item of infra) {
    ...
    assert.equal(
      result.verdict,
      "exclude",
      `${item.slug} scored ${result.score} (${result.signals.join(", ")}) — infrastructure must be excluded`
    );
  }
  ...
  assert.ok(
    design.score > worstInfra,
    `design (${design.score}) should outscore all infra (max ${worstInfra})`
  );
});
...
test("negative terms outweigh an incidental design mention", () => {
  ...
  const result = scoreRelevance({
    ...
    summary:
      "Deploy your design system documentation to Kubernetes with terraform and a CI/CD pipeline.",
  });
  assert.notEqual(
    result.verdict,
    "include",
    `scored ${result.score} (${result.signals.join(", ")}) — deployment work should not be auto-included`
  );
});
...
test("design-tool owners get a trust boost even with a neutral name", () => {
  ...
  assert.ok(
    withOwner.score > withoutOwner.score,
    "figma ownership should raise the score"
  );
  assert.ok(
    withOwner.signals.some((s) => s.startsWith("owner:")),
    "owner signal should be recorded"
  );
});
...
test("search battery uses multi-word queries only", () => {
  ...
  for (const query of QUERY_BATTERY) {
    assert.ok(
      ...
      `"${query}" is single-word and would fuzzy-match instead of semantic-match`
    );
  }
});
