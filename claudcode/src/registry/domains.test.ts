...
import { scoreDomains, DOMAINS, type Domain } from "./domains.js";
...
test("brand and copy skills are found, not just visual ones", () => {
  for (const item of MARKETING) {
    ...
    assert.notEqual(
      result.verdict,
      "exclude",
      `${item.slug} scored ${result.total} (${result.signals.join(", ")}) — should surface for review`
    );

    if (item.expect) {
      assert.ok(
        result.domains.some((d) => d.domain === item.expect),
        `${item.slug} should match domain "${item.expect}", got [${result.domains.map((d) => d.domain).join(", ")}]`
      );
    }
  }
});
...
test("skills spanning copy and visual report both domains", () => {
  ...
  assert.ok(
    popup.domains.length >= 2,
    "a genuinely multi-domain skill must not be collapsed into one bucket"
  );
});
...
test("brand identity work scores on the brand domain", () => {
  for (const name of ["brandkit", "brand-copywriter", "brand-guidelines", "naming-house"]) {
    ...
    assert.ok(
      result.domains.some((d) => d.domain === "brand" || d.domain === "copy"),
      `${name} should hit brand or copy, got [${result.domains.map((d) => d.domain).join(", ")}]`
    );
  }
});
...
test("expanding to brand and copy did not let infrastructure through", () => {
  ...
  for (const item of infra) {
    ...
    assert.equal(
      result.verdict,
      "exclude",
      `${item.name} scored ${result.total} (${result.signals.join(", ")})`
    );
  }
});
...
test("outbound email tooling is detected as copy but stays below the bar", () => {
  ...
  assert.ok(
    result.domains.some((d) => d.domain === "copy"),
    "should still be recognised as copy work rather than invisible"
  );
...
test("query battery covers every domain", () => {
  ...
  for (const q of QUERY_BATTERY) {
    assert.ok(
      ...
      `"${q}" is single-word and would fuzzy-match instead of semantic-match`
    );
  }
});
