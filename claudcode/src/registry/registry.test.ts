...
test("curated set is well formed", () => {
  ...
  for (const s of CURATED) {
    ...
    assert.ok(
      s.installCommand.startsWith("npx skills add "),
      `bad install command: ${s.slug}`
    );
  ...
  for (const s of CURATED) {
    if (s.status === "flagged" || s.status === "watch") {
      assert.deepEqual(
        s.recipes,
        [],
        `${s.slug} is ${s.status} but appears in a recipe`
      );
    }
  }
});
  ...
  for (const s of CURATED) {
    ...
    if (failing.length) {
      assert.equal(
        s.status,
        "flagged",
        `${s.slug} fails ${failing.map((a) => a.provider).join(", ")} but is marked ${s.status}`
      );
    }
  }
});
...
test("unverified install counts are null, never zero", () => {
  for (const s of CURATED) {
    assert.ok(
      s.installs === null || s.installs > 0,
      `${s.slug} has installs=0, which reads as a real measurement`
    );
  }
});
...
test("recipes resolve to real skills", () => {
  ...
  for (const r of all) {
    ...
    for (const s of r.skills) assert.ok(findSkill(s.slug));
  }
});
...
test("import writes skill files and records them in the manifest", async () => {
  ...
  assert.ok(
    !record.files.some((f) => f.includes("..")),
    "traversal path must be neutralised"
  );

  const skillMd = await readFile(
    path.join(repo, record.dir, "SKILL.md"),
    "utf8"
  );
...
test("import refuses skills outside the curated set", async () => {
  ...
  await assert.rejects(
    () => importSkill(repo, "some-random-skill"),
